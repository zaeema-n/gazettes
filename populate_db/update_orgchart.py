...
def load_transactions():
    ...
    base_folder = os.path.join("..", "data/ranil_gazettes/2022-10-05")  # Adjust path if needed
    #base_folder = os.path.join("..", "2019-12-31_2")  # Adjust path if needed
...
def rename_minister(tx, transaction, entity_counters):
    try:
        ...
        query_transfer = """
        MATCH (old:minister {name: $old})-[r:GOVERNS]->(d)
        WHERE r.type = 'AS_DEPARTMENT' AND r.end_date IS NULL
        MATCH (new:minister {name: $new})
        MERGE (new)-[:GOVERNS {type: 'AS_DEPARTMENT', start_date: date($start_date)}]->(d)
        """
...
def add_entity(tx, transaction, entity_counters):
    try:
        ...
        rel_info = transaction["rel_type"].split(":")
        rel_type = rel_info[0]  # Main relationship type
        rel_subtype = rel_info[1] if len(rel_info) > 1 else ""
        transaction_id = transaction["transaction_id"]
        ...
        MERGE (parent)-[:{rel_type} {{type: $rel_subtype, start_date: date($start_date)}}]->(child)
        """
        result = tx.run(query_create_relationship, parent=parent, child=child, start_date=date, rel_subtype=rel_subtype)
...
def move_department(tx, transaction):
    try:
        ...
        MERGE (new_parent)-[:GOVERNS {type: "AS_DEPARTMENT", start_date: date($start_date)}]->(child)
        """
        ...
        terminate_relationship_transaction = {
            ...
            "rel_type": f"GOVERNS:AS_DEPARTMENT"
        }
...
def merge_ministers(tx, transaction, entity_counters):
    try:
        ...
        for old_minister in old_ministers:
            ...
            MATCH (old:minister {name: $old})-[r:GOVERNS]->(dept:department)
            WHERE r.type = "AS_DEPARTMENT" AND r.end_date IS NULL
            MATCH (new:minister {name: $new})
            MERGE (new)-[:GOVERNS {type: "AS_DEPARTMENT", start_date: date($date)}]->(dept)
            """
...
def merge_departments(tx, transaction, entity_counters):
    try:
        ...
        MATCH (minister:minister)-[rel:GOVERNS]->(old:department {name: $old})
        WHERE rel.type = "AS_DEPARTMENT" AND rel.end_date IS NULL
        WITH minister, old
        MATCH (new:department {name: $new})
        MERGE (minister)-[:GOVERNS {type: "AS_DEPARTMENT", start_date: date($date)}]->(new)
        """
...
def terminate_entity(tx, transaction):
    try:
        ...
        # Extract main relationship type and subtype
        rel_info = transaction["rel_type"].split(":")
        main_rel_type = rel_info[0]  # Always "GOVERNS"
        rel_subtype = rel_info[1] if len(rel_info) > 1 else ""

        # Match the parent and child nodes and the specified relationship
        query_terminate_relationship = f"""
        MATCH (parent:{parent_type} {{name: $parent}})-[rel:{main_rel_type}]-(child:{child_type} {{name: $child}})
        WHERE rel.type = $rel_subtype AND rel.end_date IS NULL
        SET rel.end_date = date($end_date)
        """
        result = tx.run(query_terminate_relationship, parent=parent, child=child, end_date=date, rel_subtype=rel_subtype)
